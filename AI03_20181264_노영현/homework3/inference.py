...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #print(LIST1)
    variableDomainsDict[PAC] = LIST1
    variableDomainsDict[GHOST0] = LIST1
    variableDomainsDict[GHOST1] = LIST1
    ...
    variableDomainsDict[OBS0] = LIST2
    variableDomainsDict[OBS1] = LIST2
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        CFL = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for x in eliminationOrder:
            CFL, JF = joinFactorsByVariable(CFL, x)
            if  len(JF.unconditionedVariables()) > 1:
                CFL.append(eliminate(JF, x))
        return normalize(joinFactors(CFL))
        "*** END YOUR CODE HERE ***"
